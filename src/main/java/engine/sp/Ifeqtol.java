...
/**
 * This is the implementation for the Ifeqtol instruction.
 * The Ifeqtol intsruction is a conditional call instruction which:
 * will change the instruction pointer to the first instruction of the
 * subroutine identified by the label within the operands if and only if
 ...
 */
public final class Ifeqtol extends InstOneOperand {
     ...
     */
    public Ifeqtol() throws SPException  {
    ...
    protected boolean operandCompare(EDITBigDecimal resultAdd, EDITBigDecimal resultSub, EDITBigDecimal operand1){
        ...
        if ((operand1.isLT(resultAdd))  &&  (operand1.isGT(resultSub)))
        {
            return true;
        }
